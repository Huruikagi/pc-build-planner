...
description: Record a successful Kiro implementation-validation checkpoint in `.kiro/steering/roadmap.md`. Use immediately after `/kiro-validate-impl` returns a successful verdict, including localized report labels whose value begins with `GO` and may include a `VERIFIED` suffix. Accepts an optional feature name only as an explicit target override.
allowed-tools: Read, Write, Edit, Bash, Grep, Glob
argument-hint: [feature-name]
---

# kiro-record-validation Skill

## Core Mission
- **Success Criteria**:
  - Append exactly one validation checkpoint row to `.kiro/steering/roadmap.md`
  - Record only verdicts backed by a fresh, qualifying `/kiro-validate-impl` report
  - Never convert an unsuccessful or uncertain validation into `GO`

## Execution Steps

### Step 1: Resolve the validation target
- When `$ARGUMENTS` is empty, derive the feature from the single most-recent `/kiro-validate-impl` report in the current conversation. This is the default path.
- When `$ARGUMENTS` is provided, treat it as an optional explicit target override and require it to match the feature named by the most-recent qualifying report.
...
### Step 2: Verify eligible evidence
...
## Output Format
Use the language specified in `.kiro/specs/<feature>/spec.json`.
...
- COMMIT: <sha-or-sha+dirty>
- EVIDENCE: <concise evidence>
```

When recording is refused, report instead:

```md
## Validation Checkpoint Not Recorded
- REASON: <ambiguous report | non-GO verdict | missing evidence | stale repository state | malformed roadmap>
