...
- **Key Findings**:
  ...
  - backup-restoreの業務状態・交換形式・maintenance制御はfeature内に閉じている。React componentの横断importではなく公開mount portを抽出すれば、settingsは配置だけを所有できる。
  - `backupRestore` feature idはshell document内の選択とテスト識別子にだけ使われ、永続化モデルには格納されていない。常設feature idを`settings`へ変更しても利用者データ移行は不要である。
  - 上流`transient-feature-surface`は`presentation?: "persistent" | "transient"`を導入し、省略時をpersistentとする。settingsは既定のpersistent登録として追加できる。
...
- **Implications**:
  - backup-restoreに`BackupRestoreSectionMount`を公開し、既存registrationのstate構築とroot lifecycleをこのadapterへ移す。
  - settingsは`mount(context)`だけを呼び、backupのstate、service、file、FoundationDataPortを解釈しない。
...
- **Implications**:
  - 常設登録は`settings`へ変更し、`nav.settings`とorder 60を使用する。
  - `backupRestore`の独立登録、`nav.backupRestore`、root public key、旧E2E locatorを削除する。
...
- `backupRestore`文字列の取り残しで二重navigationや古いlocatorが残る — catalog parity、root API snapshot、boundary検索、E2E locator gateで削除を確認する。
- bilingual hintが通常の翻訳規則と競合する — ja/enカタログの双方へ同一の短い二言語値を置き、例外状態だけで表示する。
