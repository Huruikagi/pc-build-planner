...
- **対象内**: 一過性featureの登録区分、ナビゲーションからの除外、ジェスチャー起動要求、対象featureへの起動状態の表示前配送、対象タブと起動世代の固定、遷移・更新・閉鎖・常設選択による終了、戻り先、汎用的な引き渡し終了、Chrome runtime adapter、決定的検証。
...
8. When 一過性featureを起動する, the 一過性表示面 shall 対象featureが起動世代、表示対象、固定タブを検証して受理した後にだけそのfeatureを提示する
9. When 一過性featureが初めて提示される, the 一過性表示面 shall 受理済みの起動世代と固定タブに対応する実行可能な状態を利用できるようにする
10. If 対象featureが起動状態を拒否する、または起動状態の配送に失敗する, the 一過性表示面 shall そのfeatureを提示せず常設featureの利用可能な表示を維持する
