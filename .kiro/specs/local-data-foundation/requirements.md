...
- **Out of scope**: 管理UI、商品ページ解析、候補・構成の業務操作、互換性規則、JSONファイルの入出力、サイト別処理、表示言語などドメイン外の利用者インターフェース設定の保存。
- **Adjacent expectations**: 後続機能は本仕様のデータ契約と保存操作を利用するが、各機能固有の業務規則とユーザー表示を本基盤へ持ち込まない。バックアップ復元機能はJSONファイルの入出力を所有し、本基盤の評価・置換・保守契約を利用する。表示言語のようなドメイン外の利用者インターフェース設定は、本基盤の保存ポートを経由しない独立した保存領域を用いてよく、本基盤はその整合性・容量・スキーマ版数を保証しない。
