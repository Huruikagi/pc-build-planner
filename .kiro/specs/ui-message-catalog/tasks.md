...
- remediation-5: 数量formatterの架空fixtureだけでは実カタログの単複誤りを検出できないため、英語の`backup.restoreCompleted`を`MultiPluralDefinition`へ移し、公開resolverで0／1／2の3数量直積27組合せを直接検証する回帰テストを追加した。日本語の単純文字列形式とcallerのparameter契約は維持している。
