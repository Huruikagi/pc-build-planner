...
- [x] 1.3 対応言語レジストリと公開面の確定
  ...
  - ui-message-catalogの公開入口が所有する対応言語の型・集合・原語表記・言語別resolverを、ui-languageがconsumer/state境界として利用する。供給経路と記述子の形は変更しない
...
- [x] 6.4 保存 API 到達点の境界検査
  - `chrome.storage.local`への到達を保存基盤adapterと表示言語portの2ファイルへ限定し、後続`transient-feature-surface`の`chrome.storage.session`は専用runtime adapterだけへ許可する規則を、既存の公開境界検査へ追加する
