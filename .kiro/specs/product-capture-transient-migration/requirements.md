...
5. Where Chrome 116以降の未パッケージ拡張で実行される場合, the 商品取り込み移行 shall action後のdurable activation受信から候補編集面への引き渡し、および対象タブ失効または常設ナビゲーション選択による一過性面の終了・常設面復帰をproduction buildの自動E2Eで検証可能にし、ブラウザーtoolbar iconによる起動と`activeTab`付与は同じbuildに対する必須manual smokeとして分離する
