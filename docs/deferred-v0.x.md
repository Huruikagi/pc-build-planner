...
| DEF-012 | backup-restore / 4.3 | View の finalize / refresh / retry 操作は最小実装で、文言粒度と区画構成の完成は task 4.6 が所有する | UI 文言の細部 | v0.x / task 4.6 | 未着手 | 4.6 |
| DEF-013 | backup-restore / 4.2 | commit後のsection再mount時の pending finalization 照会による状態再水和は未実装 | task 4.4 の受け入れ基準そのものであり、4.2 の欠陥ではない | v0.x / task 4.4 | 未着手 | 4.4 |
