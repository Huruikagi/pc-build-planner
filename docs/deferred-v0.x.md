...
| DEF-017 | backup-restore / 5.2 | production-shaped 回復 integration は clean な `committed` outcome までを固定し、degraded 経路の finalize-only / refresh-only retry は含めない | 5.2 の受け入れ基準は finalization 完了後の通常 projection 復帰であり、単独 retry は task 3.3 / 4.4 が所有し E2E 6.3 で再確認する | v0.x / task 6.3 | 対応済 | 6.3 |
| DEF-018 | backup-restore / 6.2 | `e2e/support/backup-restore-harness.ts` は project-context の replacement guard と refresh を stub で置き、実 `ProjectContextService` を合成しない。guard lifecycle の実挙動は unit / integration test 側で担保している | 将来の拡張を見越した抽象化・test harness の構造。E2E が固定するのは backup 側の呼び出し順序と root write 回数であり、guard 内部規則は project-context spec の所有範囲 | v1.0 | 未着手 | |
| DEF-019 | backup-restore / 6.2 | `e2e/models/settings.ts` の `restoreFileInput` は `restoreFileControl` の別名に退化しており、呼び出し側を揃えれば片方を削除できる | 命名・構造の好みに属するリファクタリング | v1.0 | 未着手 | |
| DEF-020 | product-capture-transient-migration / 7.1 | candidate-management が project を作成・削除しても project-context の catalog が再読込されないため、同一 panel session 内では current context が `empty` のまま残り、以降の capture handoff が binding ではなく project-required（pending pre-edit）へ落ちる | 承認済み契約どおりの回復経路であり draft は失われない（要件 1.6 / 4.6）。production composition で current context を新鮮に保つ配線は task 7.6 が所有する | v0.x / task 7.6 | 未着手 | |
| DEF-021 | product-capture-transient-migration / 7.3 | `e2e/duplicate-product-merge.spec.ts` と `e2e/models/candidate-management.ts` の pending 選択導線は 7.1 の current-context authority に合わせた暫定更新で、E2E locator の最終整理は 7.7 の gate task が持つ | 承認済み受け入れ基準の変更ではなく、既存 gate を新しい受理契約へ追随させた test 側の移行 | v0.x / task 7.7 | 未着手 | |
| DEF-022 | compatibility-checking / 7.2 | `CompatibilityState.stop()` は context cleanup callback が throw すると、進行中評価の無効化前に終了し、失敗 resource の再 cleanup ownership も保持しない | 通常操作で到達しない稀な異常系。通常 unmount、拒否 policy、二重 unmount、承認済み lifecycle 要件は充足 | v1.0 | 未着手 | |
| DEF-023 | local-data-library-boundaries / 6.1 | `transaction.ts` にコメントアウト済みの旧 transaction pipeline 実装が残っている | 命名・構造の好みに属するリファクタリング。現行動作、公開境界、承認済み受け入れ基準には影響しない | v1.0 | 未着手 | |
